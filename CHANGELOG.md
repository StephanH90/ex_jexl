...
### Fixed
- Nested property and bracket access now resolve atom-keyed maps. Previously only the top-level identifier lookup tried atom keys; `user.name` / `user["name"]` against `%{user: %{name: "Alice"}}` returned `nil`.

### Documentation
- Documented the `intersects` array-overlap operator.
- Noted that built-in `join` and `split` use a hardcoded `,` delimiter.
- Trimmed marketing language from the Hex package description.
...
### Fixed
- `stringify` returns `nil` for values JSON cannot encode rather than raising.
- `mapby` preserves explicit `false`/`nil` values in objects (was treating them as missing keys).
- `ExJexl.AST.find_transforms/2` now recurses into transform argument expressions.
- `min`/`max`/`sum`/`avg` rescue `ArithmeticError`/`ArgumentError` and return `nil` on bad input.
